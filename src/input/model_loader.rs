    ...
    fn load_ply(filename: String) -> Option<Box<dyn object::Shape>> {
        if let Ok(mut file) = File::open(filename) {
            ...
            if let Ok(ply) = ply {
                ...
                for elem in &ply.payload["vertex"] {
                    ...
                    points.push(TriangleMeshVertex {
                        point: Point3::new(x, y, z),
                    });
                }

                let mut triangles = Vec::new();
                for elem in &ply.payload["face"] {
                    let ind = elem
                        .get_list_int(&String::from("vertex_indices"))
                        .unwrap_or(&[0, 0, 0]);
                    ...
                    triangles.push(TriangleMeshTriangle {
                        vertices: indices,
                        normal: normal,
                    });
                }
