    ...
    fn compute_bounds(
        ...
        if u_max - u_min == 1 && v_max - v_min == 1 {
            ...
            if u_max - u_min >= v_max - v_min {
                ...
                new_index = Self::compute_bounds(
                    vertices, width, height, nodes, u_split, v_min, u_max, v_max,
                );
            } else {
                let v_split = (v_min + v_max) / 2;
                Self::compute_bounds(vertices, width, height, nodes, u_min, v_min, u_max, v_split);
                new_index = Self::compute_bounds(
                    vertices, width, height, nodes, u_min, v_split, u_max, v_max,
                );
            }
...
impl Shape for Grid {
    ...
    fn intersect(&self, ray: Ray, max_distance: f32, closest: bool) -> Option<ShapeIntersection> {
        ...
        let mut func = |index: usize, max_distance: f32| {
            ...
            let surface_point0 = Point2::new(
                (u as f32) / (self.width as f32),
                (v as f32) / (self.height as f32),
            );
            let vertex1 = self.vertex(u + 1, v);
            let surface_point1 = Point2::new(
                ((u + 1) as f32) / (self.width as f32),
                (v as f32) / (self.height as f32),
            );
            let vertex2 = self.vertex(u, v + 1);
            let surface_point2 = Point2::new(
                (u as f32) / (self.width as f32),
                ((v + 1) as f32) / (self.height as f32),
            );
            let vertex3 = self.vertex(u + 1, v + 1);
            let surface_point3 = Point2::new(
                ((u + 1) as f32) / (self.width as f32),
                ((v + 1) as f32) / (self.height as f32),
            );

            if let Some((tu, tv, d)) =
                Triangle::intersect(ray, vertex0.point, vertex1.point, vertex2.point, distance)
            {
                let normal =
                    vertex0.normal * (1.0 - tu - tv) + vertex1.normal * tu + vertex2.normal * tv;
                let tangent =
                    vertex0.tangent * (1.0 - tu - tv) + vertex1.tangent * tu + vertex2.tangent * tv;
                let surface_point =
                    surface_point0 * (1.0 - tu - tv) + surface_point1 * tu + surface_point2 * tv;
            ...
            if let Some((tu, tv, d)) =
                Triangle::intersect(ray, vertex3.point, vertex2.point, vertex1.point, distance)
            {
                let normal =
                    vertex3.normal * (1.0 - tu - tv) + vertex2.normal * tu + vertex1.normal * tv;
                let tangent =
                    vertex3.tangent * (1.0 - tu - tv) + vertex2.tangent * tu + vertex1.tangent * tv;
                let surface_point =
                    surface_point3 * (1.0 - tu - tv) + surface_point2 * tu + surface_point1 * tv;
