...
impl<const N: usize> MipMap<N> {
    pub fn new(width : usize, height : usize, values : Vec<f32>) -> MipMap<N> {
        return MipMap {width, height, values};
    }
...
fn resample_mipmap<const N : usize>(mipmap : MipMap<N>, new_size : usize, horizontal : bool) -> MipMap<N> {
    ...
    for j in 0..fixed_size {
        for i in 0..new_size {
            ...
            for ii in ceil - a ..= floor + a {
                let u = si - (ii as f32);
                let lanczos = if u == 0.0 { 1.0 } else {
                    af * (PI * u).sin() * (PI * u / af).sin() / (PI * PI * u * u)
                };

                for c in 0..N {
                    if ii >= 0 && ii < old_size as i32 {
                        let sample = if horizontal { mipmap.at(ii as usize, y, c) } else { mipmap.at(x, ii as usize, c) };
...
impl<const N: usize> Texture<N> {
    pub fn new(width : usize, height : usize, values : Vec<f32>) -> Texture<N> {
        ...
        return Texture {width, height, mipmaps};
    }
    ...
    pub fn generate_mipmaps(&mut self) {
        ...
        while self.mipmaps[self.mipmaps.len() - 1].width > 1 || self.mipmaps[self.mipmaps.len() - 1].height > 1 {
    ...
    fn select_mipmap(&self, sample_projection : Bivec2) -> f32 {
        let projection_size = sample_projection.u.mag2().min(sample_projection.v.mag2()).sqrt();
