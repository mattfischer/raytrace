...
const SPLIT_PLANES : [Vec3; NUM_SPLIT_PLANES] = [Vec3::new(1.0, 0.0, 0.0), Vec3::new(0.0, 1.0, 0.0), Vec3::new(0.0, 0.0, 1.0)];

fn build_kd_tree(centroids : &[Point3], tree : &mut Vec<TreeNode>, indices : &mut [usize], split_index : usize) -> usize {
    ...
    if indices.len() == 1 {
        ...
        tree[node_index].index = build_kd_tree(centroids, tree, &mut indices[split_point..], (split_index + 1) % NUM_SPLIT_PLANES) as i32;
    }
